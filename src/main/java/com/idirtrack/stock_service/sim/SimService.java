...
public class SimService {
    ...
    public BasicResponse searchSIMsByPhoneAndCCID(String term, int page, int size) {
        Pageable pageable = PageRequest.of(page - 1, size);
        Page<Sim> simPage = simRepository.findAllByStatusAndPhoneContainingOrCcidContaining(SimStatus.PENDING, term, pageable);
    ...
    public BasicResponse updateSim(Long id, SimUpdateRequest simUpdateRequest) throws BasicException {
        ...
        if (!existingSim.getPhone().equals(simUpdateRequest.getPhone()) && simRepository.existsByPhone(simUpdateRequest.getPhone())) {
            throw new BasicException(BasicResponse.builder()
                    .content(null)
    ...
    public BasicResponse searchNonInstalledSims(String query, int page, int size) throws BasicException {
        Pageable pageable = PageRequest.of(page - 1, size);
        Page<Sim> simPage = simRepository.findAllByStatusAndPhoneContainingOrCcidContaining(SimStatus.PENDING, query, pageable);
